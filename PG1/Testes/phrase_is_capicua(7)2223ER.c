...
bool phrase_is_capicua(char words[]){
    int tam = strlen(words);
    char words_invertida[tam+1];

    // Modifica o conteudo da string
    strcpy(words_invertida, words);

    // Remove espaços da frase invertida
    int tamSpace = 0;
    for (int f = 0; f <= tam; f++) {
        if (words_invertida[f] != ' ') {
            words_invertida[tamSpace] = words_invertida[f];
            tamSpace++;
        }
    }
    words_invertida[tamSpace] = '\0';

    return word_is_capicua(words_invertida);
...
int main() {
    ...
    //Questão 7
    char frase3[] = "anotaram a data da maratona";
    bool b3 = phrase_is_capicua(frase3);

    printf("A frase e capicua? %s\n", b3 ? "Sim" : "Não");
